...
transform::transform(const Shape& sh) // конструктор класса transform
{
    shape = sh; // инициализация полей класса transform
}

Shape transform::shift(int m, int n, int k) // сдвиг фигуры на m, n, k
{
    switch (shape.getType()) // определение типа фигуры
    {
    case Shape::line: // если тип фигуры = line
        shape.x1 += m; shape.y1 += n; // сдвигаем координаты первой точки
        shape.x2 += m; shape.y2 += n; // сдвигаем координаты второй точки
        break;
    case Shape::sqr: //
...
Shape transform::scaleX(int a) // масштабирование по оси Х
{
    switch (shape.getType()) // определение типа фигуры
    {
    case Shape::line:
        shape.x1 *= a; // масштабируем координаты первой точки
        shape.x2 *= a;
        break;
    case Shape::sqr:
        shape.x1 *= a;
...
Shape transform::scaleY(int d) // масштабирование по оси У
{
...
Shape transform::scaleZ(int e) // масштабирование по оси Z
{
    switch (shape.getType())
    {
    case Shape::line:
